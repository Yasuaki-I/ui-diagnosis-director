    ...
    for i, st in enumerate(stages):
        ...
        if dense and i >= 1:
            ...
            if cv is not None:
                ...
                if need <= left_usable:
                    add_text(slide, x - 38 - int(need), y + 2, int(need) + 6,
                             ctxt, 14, bold=True,
                             color=hex_to_rgb(palette['accent']), height_px=22,
                             align=_pp_right())
                    drawn += 1
                else:
            ...
            if need <= right_usable:
                add_text(slide, x + w + 8, y + 2, int(need) + 6, dtxt, 14,
                         bold=True, color=hex_to_rgb(palette['midtone']),
                         height_px=22)
                drawn += 1
            else:
