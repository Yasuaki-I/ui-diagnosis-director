...
def draw_pyramid(slide, palette, data):
    ...
    for i, lv in enumerate(levels):
        ...
        if desc:
            sub.append(desc)
        if sub:
            # v17.2.8｜9/16 21:00 統括判定 §2-2：
            #   説明文が1行に収まる場合は中央揃えを維持し、
            #   2行以上に折り返す場合のみ左端揃えへ切り替える。
            # 根拠: 1行では左端揃えの効用（視線の戻り先の一定化）が働かず、
            #       中央配置の見出しに対し右側余白が目立ち非対称となるため
            #       （9/16 入江さんの実機確認で検出）。
            # v17.2.7 は行数に関わらず常に左端揃えであった。
            _sub_txt = '　'.join(sub)
            _sub_w = w - pad * 2
            _sub_align = (_pp_center() if _v17_fits_one_line(_sub_txt, _sub_w)
                          else _pp_left())
            add_text(slide, x + pad, y + 8 + int(label_size * 1.5), _sub_w,
                     _sub_txt, 14, color=fg,
...
def _pp_left():
    """v17.2.6｜左端揃え（9/15 入江さんのご指摘｜複数行時の段差解消）"""
    from pptx.enum.text import PP_ALIGN
...
# ---------------------------------------------------------------------
# ---------------------------------------------------------------------
# v17.2.4｜順3｜14pt太字テキストの実描画幅推定器
# 2026-09-13 の画素実測（実座標1280×720・PPTX実描画→PNG→インクBBox）から
# 最小二乗でクラス別幅を確定させた（判断原理18｜推定で決めない）。
...
def _v17_text_w14(text):
    """14pt太字で描画した際の実幅（px｜実座標1280×720）を安全側で返す。"""
    import unicodedata
    total = 0.0
    for ch in str(text):
        if ch in '0123456789':
            ...
            k = 'nar'
        total += _V17_W14[k]
    return total * _V17_W14_SAFE


def _v17_fits_one_line(text, avail_px):
    """v17.2.8｜14pt本文が枠幅 avail_px に1行で収まるかを判定する。

    9/16 21:00 統括判定 §2-2 に基づく分岐（説明文1行＝中央／2行以上＝左端）の
    判定に用いる。

    🔴 _v17_text_w14 の安全係数（_V17_W14_SAFE）を意図的に除く。
       安全係数は「枠を広く確保する」ための安全側バイアスであり、
       折返しの「判定」に用いると 1行に収まる文字列を 2行と誤判定し、
       1行なのに左端揃えになる（＝本判定が是正を求めた事象）が残るため。
       用途が逆方向であることを 9/17 に実機24点で検証した
       （安全係数あり 22/24 → 素の推定幅 24/24）。
    """
    return (_v17_text_w14(text) / _V17_W14_SAFE) <= avail_px
...
def draw_funnel(slide, palette, data):
    ...
    [v17.2.4／2026-09-13｜順3 密度強化]
      value 指定時に段間指標を左右の空きへ描画する（後方互換：value 無しは
      現行と同一出力）。V17_AREA は変更しない（全12パターン共通のため）。
      ①転換率＝左の空き／②離脱量＝右の空き／④打ち手＝最大離脱段の右／
      ⑤実数換算＝最下段の右。
      配置幅は 9/13 の画素実測（実座標1280×720・14pt太字）に基づく。
      ③通算指標・⑥注記は縦方向の空きが上下14pxしかなく V17_AREA 変更を
      要するため見送り（9/12 統括判定＝案C／v3.7候補へ登録）。
    """
    ...
    # --- v17.2.4｜段間指標の事前算出（実数 value がある場合のみ） -------
    vals = []
    for st in stages:
        v = st.get('value')
        try:
            ...
            vals.append(None)
    dense = all(v is not None and v >= 0 for v in vals) and len(vals) >= 2 \
        and vals[0] > 0
    conv, drop = [], []
    if dense:
        for i in range(1, n):
            prev, cur = vals[i - 1], vals[i]
            conv.append(int(round(cur / prev * 100)) if prev > 0 else None)
            drop.append(prev - cur)
        # 最大離脱段（離脱量が最大となる遷移の「到達側」index）
        max_i = max(range(len(drop)), key=lambda k: drop[k]) if drop else None
    else:
        max_i = None
    ...
    def _fmt(x):
        # v17.2.4｜桁区切りを入れる（9/13 目視確認で「▲27000件」を検出）
    ...
    for i, st in enumerate(stages):
        ...
        if line2 and band_h >= 56:
            # v17.2.8｜9/16 21:00 統括判定 §2-2 の基準を funnel にも適用する。
            #   1行に収まる場合は中央揃え、2行以上なら左端揃え。
            # v17.2.6 は行数に関わらず常に左端揃えであった。
            _l2_txt = '　'.join(line2)
            _l2_w = w - pad * 2
            _l2_align = (_pp_center() if _v17_fits_one_line(_l2_txt, _l2_w)
                         else _pp_left())
            add_text(slide, x + pad, y + 34, _l2_w, _l2_txt,
                     14, color=txt_color, height_px=band_h - 42,
                     align=_l2_align)
        drawn += 1

        # ----- v17.2.4｜段間指標（i>=1 の段に対して描画） --------------
        if dense and i >= 1:
            k = i - 1
            # 左：①転換率（段番号の右外側｜利用可能幅 = x - 40 - 38）
            # 段番号は num_x=x-34・幅30px を占有するため、その左外側に置く。
            # 離隔8px を確保する（9/13 目視確認でカード左辺への接触を検出）。
            left_usable = x - V17_AREA['left'] - 38
            cv = conv[k]
            if cv is not None:
                ctxt = '%d%%' % cv
                need = _v17_text_w14(ctxt)
                if need <= left_usable:
                    # v17.2.6｜9/15 実機目視：メイリオ14pt太字が枠45pxで
                    # 「35」「%」へ折返す事象を検出。word_wrap を OFF にし
                    # 比率は必ず1行で表記する（枠余裕も +6→+12 に拡大）。
                    _b = add_text(slide, x - 38 - int(need), y + 2,
                                  int(need) + 12, ctxt, 14, bold=True,
                                  color=hex_to_rgb(palette['accent']),
                                  height_px=22, align=_pp_right())
                    _b.text_frame.word_wrap = False
                    drawn += 1
                else:
                    notes.append('段%d｜転換率を省略（空き %dpx＜必要 %dpx）'
                                 % (i + 1, left_usable, int(need)))
            # 右：②離脱量
            right_usable = (V17_AREA['left'] + V17_AREA['width']) - (x + w) - 8
            dtxt = '\u25b2%s%s' % (_fmt(drop[k]), unit)
            need = _v17_text_w14(dtxt)
            if need <= right_usable:
                # v17.2.6｜①と同じ理由で1行強制（9/15 実機目視）
                _b = add_text(slide, x + w + 8, y + 2, int(need) + 12, dtxt,
                              14, bold=True,
                              color=hex_to_rgb(palette['midtone']),
                              height_px=22)
                _b.text_frame.word_wrap = False
                drawn += 1
            else:
                notes.append('段%d｜離脱量を省略（空き %dpx＜必要 %dpx）'
                             % (i + 1, right_usable, int(need)))

        y += band_h + gap

    # ----- v17.2.4｜④最大離脱段の打ち手 -------------------------------
    if dense and max_i is not None:
        tgt = max_i + 1                       # 到達側の段 index
        act = str(stages[tgt].get('action', '') or '').strip()
        if act:
            ratio_t = r_top + (r_bot - r_top) * (tgt / float(max(n - 1, 1)))
            w_t = int(V17_AREA['width'] * ratio_t)
            x_t = cx - w_t // 2
            y_t = top + (band_h + gap) * tgt
            right_usable = (V17_AREA['left'] + V17_AREA['width']) \
                - (x_t + w_t) - 8
            need = _v17_text_w14(act) + 16
            if need <= right_usable and band_h >= 56:
                # 右の空きへ枠付きで配置（離脱量の下）
                bx, by = x_t + w_t + 8, y_t + 30
                bw, bh = int(need), 30
                _v17_card(slide, bx, by, bw, bh, palette['lightest'], palette)
                add_text(slide, bx + 8, by + 6, bw - 16, act, 14, bold=True,
                         color=hex_to_rgb(palette['primary']), height_px=20)
                drawn += 2
                notes.append('最大離脱段（段%d）へ打ち手を併記' % (tgt + 1))
            else:
                # フォールバック（9/12 統括判定 §1 追加留意事項）
                # 段1・段2 は右の空きが 60〜120px しかなく枠付き併記が入らない。
                # 最下段の右（n によらず 360px の空きがある）へ「段N：打ち手」
                # の形で退避する。最下段の右は ⑤実数換算と縦に分離して置く。
                x_b = cx - int(V17_AREA['width'] * r_bot) // 2
                w_b = int(V17_AREA['width'] * r_bot)
                y_b = top + (band_h + gap) * (n - 1)
                fb_usable = (V17_AREA['left'] + V17_AREA['width']) \
                    - (x_b + w_b) - 8
                fb_txt = '段%d：%s' % (tgt + 1, act)
                fb_need = _v17_text_w14(fb_txt)
                # 最下段の右は ②離脱量（y_b+2）と ⑤実数換算（y_b+band_h-26）が
                # 既に占有するため、その中間へ置いて縦に分離する
                # （9/13 目視確認で離脱量との重なりを検出）。
                fb_y = y_b + max(26, band_h // 2 - 11)
                if fb_need <= fb_usable and fb_y + 22 <= y_b + band_h - 26:
                    add_text(slide, x_b + w_b + 8, fb_y,
                             int(fb_need) + 8, fb_txt, 14, bold=True,
                             color=hex_to_rgb(palette['accent']),
                             height_px=22)
                    drawn += 1
                    notes.append('最大離脱段（段%d）の右の空きが %dpx で'
                                 '枠付き併記に足りないため、最下段の右へ'
                                 '退避配置した（必要 %dpx／空き %dpx）'
                                 % (tgt + 1, right_usable, int(fb_need),
                                    fb_usable))
                else:
                    notes.append('最大離脱段（段%d）の打ち手を省略'
                                 '（右の空き %dpx／退避先 空き %dpx・必要 %dpx'
                                 '／退避先の縦余地 band_h=%dpx）'
                                 % (tgt + 1, right_usable, fb_usable,
                                    int(fb_need), band_h))

    # ----- v17.2.4｜⑤実数換算（最下段の右） ---------------------------
    tn = str(data.get('total_note', '') or '').strip()
    if tn:
        ratio_b = r_bot
        w_b = int(V17_AREA['width'] * ratio_b)
        x_b = cx - w_b // 2
        y_b = top + (band_h + gap) * (n - 1)
        right_usable = (V17_AREA['left'] + V17_AREA['width']) - (x_b + w_b) - 8
        need = _v17_text_w14(tn)
        if need <= right_usable:
            add_text(slide, x_b + w_b + 8,
                     y_b + band_h - 26, int(need) + 8, tn, 14, bold=True,
                     color=hex_to_rgb(palette['primary']), height_px=22)
            drawn += 1
        else:
