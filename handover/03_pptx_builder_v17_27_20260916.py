...
def draw_pyramid(slide, palette, data):
    ...
    for i, lv in enumerate(levels):
        ...
        if desc:
            sub.append(desc)
        if sub:
            # v17.2.7｜9/15 19:00 統括判定 §2（諮問①案A）：
            # 複数行になりうる本文（description）は左端揃えとする。
            # 見出し（label）・序列番号は中央揃えのまま維持する。
...
def draw_funnel(slide, palette, data):
    ...
    for i, st in enumerate(stages):
        ...
        if line2 and band_h >= 56:
            # v17.2.6｜9/15 入江さんのご指摘：説明文が複数行になると
            # 中央揃えでは行ごとに左端が変動し視線の戻り先が不定になる。
            # 説明文のみ左端揃えへ変更する（見出しは中央揃えのまま）。
