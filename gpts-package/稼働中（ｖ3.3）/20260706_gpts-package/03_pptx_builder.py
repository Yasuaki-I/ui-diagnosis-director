...
def add_proposal_onepager(prs, proposals_data, page_num=1, total=None,
    ...
    # [FB対応 2026-07-02] POINT帯を 62px に拡大し top を 620→594 に上シフト。
    POINT_BAND_TOP = 594
    AREA_BOTTOM_B = POINT_BAND_TOP - 8  # 586
...
def _visual_meta_header(slide, visual_data, top=90):
    ...
    # [FB対応 2026-07-04] 全4カード NAVY 統一（C-1側は 6/30 に統一済みだったが
    # C-3側の本関数は統一漏れしていた）。装飾目的の色分けは廃止し、値側の
    # 色分け（value_color）で個別強調は継続。
    meta_cards = [
        {'bar': NAVY, 'label': '目的',
    ...
    for i, card in enumerate(meta_cards):
        ...
        # 色帯6pxに合わせてX+2pxシフト
        _add_multi_run_box(slide, x + 18, top + 8, card_w - 26, INFO_H - 14, [
...
def add_visual_board(prs, visual_data, page_num=1, total=3, slide_no='3',
    ...
    if n_sec > 0:
        ...
        for i, sec in enumerate(sections):
            ...
            name = _truncate_full(
                sec.get('label') or sec.get('name', ''), 8)
    ...
    add_shape(slide, MSO_SHAPE.RECTANGLE, CENTER_X, SUMMARY_TOP + 28, CENTER_W,
              ...
              line=BORDER_GRAY, line_width_pt=0.5)
    # [FB対応 2026-07-06 v11] v10で24字設定にしたが _truncate_full は
    # 全角換算のため半角混じり文（例:「FV・信頼性は高水準。コピーとフォームの
    # 改善でCVR…」=27字）が2行折返しでオーバーフロー。
    # 半角混じり実データを想定し、14pt×width352pxで1行内に確実に収まる
    # 実文字数上限を厳格化。
    # [FB対応 2026-07-06 v11 追加調整] 20字設定でも半角混じり実データ22字が
    # 2行折返し発生（LibreOffice描画で「の改…」が2行目にはみ出しTop3ヘッダに接触）。
    # 14pt Meiryo × width 352px の実測描画幅を根拠に 16字へ厳格化。
    summary_text = _truncate_full(visual_data.get('summary', ''), 16)
    _add_multi_run_box(slide, CENTER_X + 14, SUMMARY_TOP + 34, CENTER_W - 28,
                       SUMMARY_H - 36, [
    ...
    # ---- 中央下：最重要課題3点 ----
    # [FB対応 2026-07-03] Top3レイアウト崩れ修正：
    # - line_height 1.5→1.35 で行間を詰める
    # - 各行 _truncate_full 30→22字 で1行内に収める
    # [FB対応 2026-07-04 v9・案C] v8 の ISSUE_H=172 では 3件×2行折返し時に
    # 内部領域 130px < 必要 156px で下端切れ発生。ISSUE_H を 172→200 に拡大し
    # 内部領域を 158px 確保して根本解消。
    ISSUE_TOP = SUMMARY_TOP + SUMMARY_H + 10
    ISSUE_H = 190  # end=158+76+10+190=434（右カラム下端と揃う）
    ...
    # 表示件数を動的判定：ISSUE_H(172) - header(28) - padding(16) = 128px
    # 1行あたり 14pt × line_height 1.35 = 約26px、番号付+1行想定=26px、
    # 2行想定=52px。安全マージン込みで3件×2行=156pxはNG、
    # 3件×1行=78pxはOK、3件×最大1.5行=117pxはギリギリOK。
    # → 22字上限で1行に収める設計（3件全表示可能）
    issue_paragraphs = [
        {'text': f'{i + 1}. ' + _truncate_full(s, 22),
         'size': 14, 'bold': False, 'color': TEXT, 'line_height': 1.35}
    ...
    # ---- 下：行動フロー6ステップ ----
    # [ビジュアル強化 2026-06-29]
    # - ステップ円：36 → 44px に拡大（視認性向上）
    # - ステータス文字：16 → 18pt
    # - 本文背景：白＋NAVY 3%極薄で奥行き演出
    FLOW_TOP = 444
    # [FB対応 2026-07-02] 改善方向帯62px化(top=594)に伴い FLOW下端を 590 に
    # 抑えるため FLOW_H を 156→146 に縮小（安全余白4px）。
    FLOW_H = 146
    FLOW_W = 1200
